...
from rvap.vap_main import EncoderCPC, GPT, GPTStereo, ObjectiveVAP
...
class VAPRealTime():
    ...
    def process_vap(self, x1, x2):
        ...
        with torch.no_grad():
            ...
            x1_ = torch.tensor(x1, dtype=torch.float32, device=self.device).unsqueeze(0).unsqueeze(0)
            x2_ = torch.tensor(x2, dtype=torch.float32, device=self.device).unsqueeze(0).unsqueeze(0)
