...
class DialogBridge(ABC):
    ...
    def get_respose(self, transcription: str, slot_states: dict):
        ...
        if self.awaiting_final_confirmation and YES in transcription:
            response = self.nlg.get_confirm_response(
                self.dst.state_stack[-1][0], YES
            )  # 最終確認応答を生成
            self.awaiting_final_confirmation = False
            self.waiting_for_confirmation = False
        else:
            nlu_output = (
                {"action_type": "新規予約", "slot": slot_states}
                if not self.dst.state_stack
                else {"action_type": "", "slot": slot_states}
            )
            # DST状態更新
            prev_state = (
                deepcopy(self.dst.state_stack[-1][1])
                if self.dst.state_stack
                else self.dst.initial_state
            )
            self.dst.update_state(nlu_output)

            # 暗黙確認応答生成
            if self.use_implied_confirmation:
                implicit_confirmation = self.nlg.get_confirmation_response(
                    self.dst.state_stack[-1], prev_state
                )
    ...
    async def send_tts(self, ws, tts_bridge, firestore_client):
        ...
        if (
            self.dst.is_complete()
            and not self.waiting_for_confirmation
            and not self.awaiting_final_confirmation
        ):
            self.is_final = True
    ...
    async def __call__(self, ws, asr_bridge, llm_bridge, tts_bridge, **kwargs):
        ...
        if turn_taking_status == TurnTakingStatus.END_OF_TURN:
            ...
            if transcription != "":
                ...
                if (
                    not self.wait_for_llm
                    and self.is_no_slots
                    and (YES not in transcription or NO in transcription)
                ):
...
class DialogBridgeWithLLMSFAndVolumeBasedEoT(DialogBridge):
    ...
    def get_bargein_flag(self) -> bool:
        return (
            self.allow_barge_in
            and len(self.streaming_vad.speech_chunks) > BARGE_IN_THRESHOLD
        )
    ...
    def turn_taking(self, *args):
        ...
        if (
            (self.is_terminal_form_detected and self.is_fast_speech_end)
...
class DialogBridgeWithVAP(DialogBridge):
    ...
    def turn_taking(self, *args):
        # VAPの結果を取得
        vap_result = self.vap_bridge.get_result()
        ...
        if (
            ...
            and p_now > 0.0
            and self.pre_text != ""
            and not self.bot_speak
        ):
            return TurnTakingStatus.END_OF_TURN
        elif 0.3 < p_future < 0.7 and p_now < 0.3 and p_future > 0.0 and p_now > 0.0:
            return TurnTakingStatus.BACKCHANNEL
        else:
            return TurnTakingStatus.CONTINUE

    def vad_step(self, chunk):
        if chunk != "/w==":  # 空のチャンクをスキップ
            # base64でエンコードされた音声データをデコード
            try:
    ...
    def get_bargein_flag(self) -> bool:
        ...
        return p_now < 0.3 and p_future > 0.7

    def terminate(self):
        """終了処理"""
        self.vap_bridge.terminate()
